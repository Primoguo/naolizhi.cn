...
## 更新摘要
**变更内容**
- 新增了5个高性能CSS关键帧动画：animate-grid-move、animate-fade-in-up、animate-flow-pulse、animate-glow-pulse、animate-line-flow
- 所有新动画均使用transform和opacity优化性能，避免重排重绘
- 在Highlights组件中广泛应用这些动画效果
- 更新了动画系统的架构说明和使用指南
...
L["src/sections/Highlights.tsx"] --> M["新动画系统<br/>grid-move/fade-in-up/flow-pulse/glow-pulse/line-flow"]
```

图表来源
- [tailwind.config.js:1-92](file://tailwind.config.js#L1-L92)
- [postcss.config.js:1-7](file://postcss.config.js#L1-L7)
- [src/index.css:1-225](file://src/index.css#L1-L225)
...
- 动画与过渡
  ...
  - **新增高性能动画系统**：包含 grid-move（网格背景移动）、fade-in-up（内容淡入）、flow-pulse（数据流脉冲）、glow-pulse（标签呼吸灯）、line-flow（渐变流动）等5个关键帧动画
- 组件样式
...
章节来源
- [src/index.css:7-225](file://src/index.css#L7-L225)
...
- **动画层**：新增的高性能动画系统通过 transform 和 opacity 优化，避免重排重绘
...
subgraph "动画层"
A1["grid-move<br/>20s网格背景"]
A2["fade-in-up<br/>内容淡入"]
A3["flow-pulse<br/>数据流脉冲"]
A4["glow-pulse<br/>标签呼吸灯"]
A5["line-flow<br/>渐变流动"]
end
subgraph "页面层"
...
**更新** 新增了5个高性能CSS关键帧动画，全部使用transform和opacity优化性能
...
**1. 网格背景移动动画 (animate-grid-move)**
- 用途：创建缓慢移动的网格背景效果
- 性能优化：使用 background-position 变换，20秒循环周期
- 应用场景：Highlights 卡片的背景装饰

**2. 内容行淡入动画 (animate-fade-in-up)**
- 用途：内容行的依次淡入效果
- 性能优化：使用 transform 和 opacity，0.6秒缓动时间
- 应用场景：模拟播放器界面的文本行动画

**3. 数据流脉冲动画 (animate-flow-pulse)**
- 用途：数据流效果的流动线脉冲
- 性能优化：仅改变 opacity，2秒循环周期
- 应用场景：流程连接线的动态效果

**4. 标签呼吸灯动画 (animate-glow-pulse)**
- 用途：标签的呼吸灯效果
- 性能优化：使用 box-shadow 变化，3秒循环周期
- 应用场景：状态标签的动态提示

**5. 渐变流动动画 (animate-line-flow)**
- 用途：连续的渐变流动效果
- 性能优化：使用 background-position 变换，2秒线性循环
- 应用场景：进度条和连接线的流动效果
...
- [src/index.css:80-225](file://src/index.css#L80-L225)
- [src/hooks/use-scroll-reveal.ts:1-34](file://src/hooks/use-scroll-reveal.ts#L1-L34)
- [src/sections/Features.tsx:34-61](file://src/sections/Features.tsx#L34-L61)
- [src/sections/Highlights.tsx:105-205](file://src/sections/Highlights.tsx#L105-L205)
...
+line-flow : "2s linear infinite"
}
Button --> Utils : "合并类名"
Card --> Utils : "合并类名"
AnimationSystem --> Components : "应用动画"
```
...
- [src/components/ui/card.tsx:1-93](file://src/components/ui/card.tsx#L1-L93)
- [src/lib/utils.ts:1-7](file://src/lib/utils.ts#L1-L7)
- [src/index.css:105-205](file://src/index.css#L105-L205)
...
- **Highlights 区域**
  - **全面应用新动画系统**：网格背景移动、内容行淡入、数据流脉冲、标签呼吸灯、渐变流动
...
- 动画与渲染
  ...
  - **新动画系统优化**：所有5个新增动画都经过性能优化，使用GPU加速的属性
    - grid-move：使用 background-position 而非 transform
    - fade-in-up：仅使用 transform 和 opacity
    - flow-pulse：仅改变 opacity，避免任何布局计算
    - glow-pulse：使用 box-shadow 变化，避免重排
    - line-flow：使用 background-position 变换
- 字体加载
...
- 动画不触发
  ...
  - **新动画问题排查**：确认动画类名正确应用，检查 animation-delay 设置
- 滚动入场无效
...
## 结论
本项目样式系统以 Tailwind CSS 为核心，通过 CSS 变量与主题映射建立统一的设计令牌，结合原子类与组件化实践，实现了高一致性与可扩展的样式架构。**新增的高性能动画系统进一步提升了用户体验**，通过精心设计的5个关键帧动画，在保持流畅性的同时避免了性能开销。动画与滚动入场通过轻量钩子与 CSS 变量驱动，兼顾性能与体验。建议在后续迭代中持续遵循语义 token、原子类组合与 cn 合并类名的约定，并保持对新动画系统的性能关注，以保持样式系统的整洁与可维护性。
...
- 动画
  ...
  - **新增高性能动画**：animate-grid-move、animate-fade-in-up、animate-flow-pulse、animate-glow-pulse、animate-line-flow
- 响应式
...
**1. 网格背景移动 (animate-grid-move)**
```html
<div className="animate-grid-move" style={{ 
  backgroundImage: 'linear-gradient(rgba(255,255,255,.5) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,.5) 1px, transparent 1px)', 
  backgroundSize: '24px 24px' 
}} />
```

**2. 内容行淡入 (animate-fade-in-up)**
```html
<div className="animate-fade-in-up" style={{ animationDelay: '0.1s' }} />
```

**3. 数据流脉冲 (animate-flow-pulse)**
```html
<div className="animate-flow-pulse" style={{ animationDelay: '0.2s' }} />
```

**4. 标签呼吸灯 (animate-glow-pulse)**
```html
<div className="animate-glow-pulse" style={{ animationDelay: '0s' }} />
```

**5. 渐变流动 (animate-line-flow)**
```html
<div className="animate-line-flow" />
```
